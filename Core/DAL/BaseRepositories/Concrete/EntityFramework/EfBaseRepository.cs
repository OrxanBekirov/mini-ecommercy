...
namespace Core.DAL.BaseRepositories.Concrete.EntityFramework
{
        ...
        where TContext : DbContext
    {
        ...
        public async Task<List<TEntity>> GetAllWithPaginationAsync(int pageNumber, int pageSize, Expression<Func<TEntity, bool>> filter, params string[] includes)
        {
          ...
          : await query.Skip((pageNumber - 1) * pageSize).Take(pageSize).Where(filter).ToListAsync();
        }


        public async Task<TEntity> GetAsync(Expression<Func<TEntity, bool>> filter, params string[] includes)
        {
            return await GetQuery(includes).FirstOrDefaultAsync(filter);
        }
